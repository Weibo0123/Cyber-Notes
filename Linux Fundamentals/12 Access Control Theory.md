...
This file covers the *conceptual model* behind access control in general. File 11 (Permissions) covers how Linux actually implements a simplified version of it.
...
⚠️ Note: this `{r,w,x,a,o}` set is the **abstract teaching model**, not literally what `chmod` gives you — real POSIX permission bits are only `r`, `w`, `x` (file 11). The theoretical model separates `write` (modify existing content) from `append` (add to the end only, can't modify existing content) and adds `own` (ability to change who has rights over the object) as distinct concepts worth understanding even though Unix doesn't expose them as separate chmod bits.
...
If a process only needs `read(A)`, it should not additionally hold `write(A)`, `read(B)`, `execute(C)`, etc. This limits the blast radius if that Subject is ever compromised or misused — a recurring theme once you get to SUID binaries (file 11) and privilege escalation generally.
